    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if(profile != None):
                if(conf <= 50):
                    cv2.putText(
                        frame, "Name : "+str(profile[1]), (x, y+h+20), font, 0.6, (255, 255, 255), 1)
                    cv2.putText(
                        frame, "Age : "+str(profile[2]), (x, y+h+45), font, 0.6, (255, 255, 255), 1)
                    cv2.putText(
                        frame, "Gender : "+str(profile[3]), (x, y+h+70), font, 0.6, (255, 255, 255), 1)
                    cv2.putText(frame, "Criminal Records : " +
                                str(profile[4]), (x, y+h+95), font, 0.6, (255, 255, 255), 1)
                else:
                    cv2.putText(frame, 'Unknown', (x, y+h),
                                font, 0.6, (255, 0, 0), 1)

            maxindex = int(np.argmax(prediction))
            cv2.putText(frame, "Emotion:"+str(emotion_dict[maxindex]), (x+20, y-60),
                        font, 1, (255, 255, 255), 2, cv2.LINE_AA)

            end_time = time.time() - star_time
            FPS = 1/end_time
            cv2.putText(frame, f"FPS: {round(FPS,3)}", (10, 50),
                        cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2)

        cv2.imshow('Video', cv2.resize(
            frame, (720, 540), interpolation=cv2.INTER_CUBIC))
